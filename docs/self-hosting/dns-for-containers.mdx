...
<Steps>
<Step title="Create bridge network with static subnet">
Create a bridge network with a static subnet for your stack.
</Step>

<Step title="Deploy AdGuard Home with static IP">
Run AdGuard Home with a fixed IP in that subnet.
</Step>

<Step title="Assign static IPs to services">
Assign static IPs to the remaining containers.
</Step>

<Step title="Point service DNS to AdGuard Home">
Set each container `dns` to the AdGuard Home static IP.
</Step>

<Step title="Control startup order">
